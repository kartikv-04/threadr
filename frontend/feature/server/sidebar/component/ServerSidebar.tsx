...
const ServerSidebar = () => {
    ...
    return (
        <>
            <aside className="flex flex-col items-center w-[72px] min-w-[72px] h-screen bg-neutral-950 py-3 overflow-y-auto overflow-x-hidden scrollbar-hide">
                {/* Home Button */}
                <ActionButton
                ...
                <div className="flex flex-col gap-2 flex-1">
                    {isPending ? (
                        ...
                        <div className="text-red-500 text-xs text-center px-2">Error loading</div>
                    ) : servers.length === 0 ? (
                        <div className="text-neutral-500 text-xs text-center px-2">No servers</div>
                    ) : (
                        servers.map((server) => (
                            <ServerIcon
                                key={server.serverId}
                                ...
                                onClick={() => setActiveServerId(server.serverId)}
                            />
                        ))
                    )}
                </div>
